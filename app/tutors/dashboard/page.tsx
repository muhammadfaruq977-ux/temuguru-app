    ...
    try {
      tutor = await prisma.tutor.create({
        data: {
          ...
          email: userEmailClean,
          phone_wa: randomPhone, // Menggunakan nomor acak unik agar lolos validasi database
      ...
      console.error("Gagal membuat profil tutor otomatis:", createError);
      redirect("/tutors/login?error=profile_creation_failed");
    }
  }

  const tutorEmailSafe = tutor?.email ? tutor.email.toLowerCase().trim() : userEmailClean;

  // 3. Pastikan record User ada di Prisma untuk relasi PushSubscription
  let prismaUser = await prisma.user.findUnique({
    where: { email: tutorEmailSafe },
  });
  ...
  if (!prismaUser) {
    try {
      prismaUser = await prisma.user.create({
        data: {
          email: tutorEmailSafe,
          name: tutor.name,
          role: "TUTOR" as Role
        },
      });
    } catch (err) {
      prismaUser = await prisma.user.findUnique({ where: { email: tutorEmailSafe } });
    }
  }
